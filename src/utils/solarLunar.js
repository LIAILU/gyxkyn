...
var solarLunar = {
  ...
  toAstro: function (cMonth,cDay) {
    ...
    solar2lunar: function (y, m, d) { //参数区间1900.1.31~2100.12.31
        ...
        //该日期所属的星座
        var astro = solarLunar.toAstro(m,d);
    ...
    formatDate : (timestamp) => {
        ...
        return year + '-' + solarLunar.addPre(month) + '-' + solarLunar.addPre(date) + ' ' + solarLunar.addPre(hours) + ':' + solarLunar.addPre(minutes) + ':' + solarLunar.addPre(seconds)
        },
     ...
     *  @param {*} timestamp 日期时间戳 
      * @returns 
      *     {
              lunarDate, // X年 x 月 x 日 星期 x  星座 （洋历）
              lunarDateSecond xx年xx月xx日 节气 生肖 （农历）
            }
      */
     getLunarDate : (timestamp) => {
            // { "lYear" : 2021, "lMonth" : 6, "lDay" : 27, "Animal" : "牛", "IMonthCn" : "六月", "IDayCn" : "廿七", "cYear" : 2021, "cMonth" : 8, "cDay" : 5, "gzYear" : "辛丑", "gzMonth" : "乙未", "gzDay" : "乙酉", "isToday" : true, "isLeap" : false, "nWeek" : 4, "ncWeek" : "星期四", "isTerm" : false, "Term" : null, "astro" : "狮子座" }
            ...
            const dayInfo = solarLunar.solar2lunar(nowYear, nowMonth, nowDate)
            const lunarDate = dayInfo['lYear'] + '年' + solarLunar.addPre(nowMonth) + '月' + solarLunar.addPre(nowDate) + '日 ' + dayInfo['ncWeek'] + ' ' + dayInfo['astro']
            const lunarDateSecond = dayInfo['gzYear'] + '年' + dayInfo['gzMonth'] + '月' + dayInfo['gzDay'] + '日 ' + (dayInfo['term'] == null ? '' : dayInfo['term']) + ' 【' + dayInfo['animal'] + '】'
            return {
              lunarDate,
              lunarDateSecond
            }
